...
class ElasticsearchProxyViewTests(SimpleTestCase):
    ...
    def test_http_basic_authentication_header_is_added_when_username_and_password_are_defined(self):
        """HTTP basic authentication header should be added when settings.ELASTICSEARCH_HA_PROXY_USERNAME and settings.ELASTICSEARCH_HA_PROXY_PASSWORD are defined."""
        with self.settings(ELASTICSEARCH_HA_PROXY_USERNAME='foo'):
            with self.settings(ELASTICSEARCH_HA_PROXY_PASSWORD='bar'):
                ...
                expected_header_value = get_basic_authentication_header_value(settings.ELASTICSEARCH_HA_PROXY_USERNAME, settings.ELASTICSEARCH_HA_PROXY_PASSWORD)
    ...
    def test_http_basic_authentication_header_is_not_added_when_username_or_password_is_not_defined(self):
        """HTTP basic authentication header should not be added when settings.ELASTICSEARCH_HA_PROXY_USERNAME and/or settings.ELASTICSEARCH_HA_PROXY_PASSWORD is None."""
        search_url = self.base_url + "publication,person/"
        with self.settings(ELASTICSEARCH_HA_PROXY_USERNAME=None):
            with self.settings(ELASTICSEARCH_HA_PROXY_PASSWORD=None):
                response = self.client.get(search_url)
                headers = self.urlopen_mock.call_args.kwargs['headers']
